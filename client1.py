...
class Window:
	def __init__(self):
		...
		pg.display.set_caption("Tic Tac Toe Client 1")
	...
	while run:
		...
		if gameOver == 0:
			...
			if len(revMov)!=0:
				...
				if posn[2]==2:
					Player2.moves((posn[0],posn[1]),Player1)
		...
		for event in pg.event.get():
			if event.type==pg.QUIT:
				...
				break	
			if gameOver==0:
				if event.type==pg.MOUSEBUTTONDOWN and clicked==False and gameOver == 0:
					clicked=True
				if event.type==pg.MOUSEBUTTONUP and clicked==True:
					clicked=False
					Player1.moves(pg.mouse.get_pos(),Player2)
					n.send(window.makePos((Player1.curpos[0],Player1.curpos[1],1)))
			else:
